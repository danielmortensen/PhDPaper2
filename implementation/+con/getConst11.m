function Const = getConst11(sim,var,Const)
...
for iBus = 1:sim.bus.nBus
    for iRoute = 1:sim.routes.nRoute(iBus)
        s2 = var.val.s2.val.onRamp(iBus,iRoute,:);
        p = var.val.p.val.onRamp(iBus,iRoute,:);
...
% package constraint
Const.Constraint11.A = A;
Const.Constraint11.b = b;
Const.Constraint11.eq = repmat('<',[nConst,1]);
Const.Constraint11.info = "constriants for discretized on-ramp average power";
